...
**Source:** http://platform.claude.com/docs/en/manage-claude/compliance-api
...
Request Compliance API access for your organization, then create a Compliance Access Key (with scoped permissions) or an Admin API key, and learn which to use.](/docs/en/manage-claude/compliance-api-access)[Query the Activity Feed

Retrieve, filter, and paginate the shared Activity Feed. Supported by both key types.](/docs/en/manage-claude/compliance-activity-feed)[Retrieve and delete chats, files, and projects

Read chat content and attachments, then delete on demand. Compliance Access Key required.](/docs/en/manage-claude/compliance-content-data)[List organizations, users, roles, and groups

Enumerate linked organizations, members, roles, and directory groups.](/docs/en/manage-claude/compliance-org-data)[Design your compliance integration

Choose a feed-consumption pattern, plan SIEM correlation, and decide your retention approach.](/docs/en/manage-claude/compliance-integration-patterns)[Handle Compliance API errors

Every 400, 401, 403, 404, 409, 429, and 5xx response the Compliance API returns, with the fix for each.](/docs/en/manage-claude/compliance-errors)[API reference

Endpoint paths, parameters, and response schemas for every Compliance API call.](/docs/en/api/compliance)[Compliance API FAQ

Answers to common key, scope, availability, and integration questions.](/docs/en/manage-claude/compliance-faq)
