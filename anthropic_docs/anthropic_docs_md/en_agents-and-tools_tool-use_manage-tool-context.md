...
**Source:** http://platform.claude.com/docs/en/agents-and-tools/tool-use/manage-tool-context
...
Load tool definitions on demand instead of upfront.](/docs/en/agents-and-tools/tool-use/tool-search-tool)[Programmatic tool calling

Collapse tool-call chains into a single executable script.](/docs/en/agents-and-tools/tool-use/programmatic-tool-calling)[Tool use with prompt caching

Cache tool definitions across requests to cut token costs.](/docs/en/agents-and-tools/tool-use/tool-use-with-prompt-caching)[Context editing

Trim stale tool results from long-running conversations.](/docs/en/build-with-claude/context-editing)
