...
**Source:** http://platform.claude.com/docs/en/manage-claude/workload-identity-federation
...
STS web identity tokens, or EKS IRSA projected tokens.](/docs/en/manage-claude/wif-providers/aws)[Google Cloud

Google-signed identity tokens via the metadata server.](/docs/en/manage-claude/wif-providers/gcp)[Microsoft Azure

Managed Identity (IMDS) and Entra Workload ID on AKS.](/docs/en/manage-claude/wif-providers/azure)[GitHub Actions

Keyless CI authentication with the Actions OIDC token.](/docs/en/manage-claude/wif-providers/github-actions)[Kubernetes

Self-managed and on-premises clusters using projected service-account tokens.](/docs/en/manage-claude/wif-providers/kubernetes)[SPIFFE

Workloads with SPIFFE JWT-SVIDs from SPIRE or another conformant issuer.](/docs/en/manage-claude/wif-providers/spiffe)[Okta

Okta service applications using client-credentials flow.](/docs/en/manage-claude/wif-providers/okta)
